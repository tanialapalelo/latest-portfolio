...
No `openGraph.images`/`twitter.images` here: `app/opengraph-image.tsx`'s file-based convention takes priority over the `metadata` object and auto-generates `og:image`/`twitter:image` (with correct type/width/height and a content hash) on its own. A manual `images` array would just be dead config the file convention overrides, the same reasoning that already ruled out a separate `twitter-image.tsx` file.
...
- `<meta property="og:image" content="...opengraph-image?..." />` (auto-generated by the file convention, resolves to an absolute URL via `metadataBase` in production)
